...
optional = [ {'label': 'All Sites', 'value': 'ALL'}]

spacex_df = pd.read_csv("spacex_launch_dash.csv")
for i in spacex_df.groupby("Launch Site").first().reset_index()["Launch Site"]:
    temp = {'label': i, 'value': i}
    optional.append(temp)
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title='Selected Success Launced by Site')
        return fig
    else:
        fig = px.pie(filtered_df[filtered_df["Launch Site"]==entered_site],
        names='class', 
        title=f'Selected Success Launced by {entered_site}')
        return fig
# TASK 4:
